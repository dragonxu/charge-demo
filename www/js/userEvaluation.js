...
var tableFunc = {
	...
	load_func:function(callback){
		var charge_user = $.trim($("#charge_user").val()),
			p_id = $("#p_id").val(),
		...
		var subData = {
			...
			score : score//支付状态，3 充电中 6-待支付 7支付成功
		};
		var userInfoList = gs.func.getUserData();
		for(var key in userInfoList){//用户名 商户名 token数据加载
			subData[key] = userInfoList[key];
		}
		gs.func.callCgi(gs.cgi.web_score_query,subData,function(json){
			console.log(json.data.score_data);
			var table_list = json.data.score_data;
			$("#table_Eva").dataTable({
				data:table_list,
	        	columns:[
	        		{title:'充电用户名',data:'user_name'},
	        		{title:'电站名',data:'station_name'},
	        		{title:'充电桩编号',data:'pile_id'},
	        		{title:'评价内容',data:'comment'},
	        		{title:'评分',data:'score',render:function(type,data,row){
	        			return row.score + '星';
	        		}},
	        		{title:'评价时间',data:'comment_time'}
	        	],
